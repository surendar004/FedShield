"""Streamlit dashboard for FedSIG+ (dark cybersecurity theme, logic preserved)."""
import os
import sys
import time
import requests
...
st.set_page_config(
    page_title="FedSIG+ | Trust-Aware Federated Mapping",
    page_icon="🛡️",
    layout="wide",
...
# Resilient requests session: retries/backoff to tolerate brief API unavailability
...
def fetch_threats(page=1, per_page=1000):
    """Fetch signature contributions with pagination support.

    On transient network errors this returns an empty DataFrame so the
    UI can render gracefully and show a single informative warning.
    """
    try:
        params = {'page': page, 'per_page': per_page}
        r = session.get(f'{API_BASE}/threats', params=params, timeout=5)
...
def fetch_summary():
    ...
    Returns an empty dict on failure so the UI can decide how to present
    a concise error message instead of raising exceptions inside Streamlit.
    """
    try:
        r = session.get(f'{API_BASE}/system_summary', timeout=5)
        if r.status_code == 200:
            return r.json()
        return {}
    except requests.exceptions.RequestException:
...
# metrics summary endpoint removed from dashboard UI; server-side endpoint remains available

# Header
st.markdown("""
<div class="main-header">
    <h1 class="main-title">🛡️ FedSIG+: Trust-Aware Federated Mapping of Suspicious File Behaviour</h1>
    <p class="main-subtitle">Federated Learning Security Dashboard • Trust-Aware Collaboration</p>
    <div class="hero-cta">
        <a class="glow-btn" href="#system-summary">View Summary</a>
        <a class="glow-btn red" href="#threats">Inspect Signatures</a>
        <a class="glow-btn" href="http://127.0.0.1:5000/api/health" target="_blank">API Health</a>
    </div>
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### ℹ️ Info")
    st.caption("FedSIG+ maps suspicious file behaviour using trust-aware federated learning. Monitor signature quality, isolation actions, and trusted contributor participation without altering the original workflow.")
...
with st.container():
    ...
    # If summary is empty, show a small non-blocking banner so users understand
    # the dashboard is waiting for the backend. This avoids noisy stack traces
    # from transient connection errors while still rendering the rest of the UI.
    if not summary:
        st.warning("API offline or unreachable — retrying in background. Ensure the backend is running at http://127.0.0.1:5000")
    metrics_cards.render(summary)
    st.markdown('</div>', unsafe_allow_html=True)

with st.container():
    st.markdown('<div id="threats" class="section"><div class="section-header">Signature Contributions</div>', unsafe_allow_html=True)
    df_full = fetch_threats()
    if not df_full.empty:
        csv_bytes = df_full.to_csv(index=False).encode('utf-8')
        st.download_button("Download signatures as CSV", data=csv_bytes, file_name="signatures.csv", mime="text/csv")
    threat_table.render(df_full)
    st.markdown('</div>', unsafe_allow_html=True)

with st.container():
    st.markdown('<div class="section"><div class="section-header threat-amber">Signature Analytics</div>', unsafe_allow_html=True)
    threats_df = fetch_threats()
    model_charts.render(threats_df, key="charts_vertical")
    st.markdown('</div>', unsafe_allow_html=True)

# Client Metrics Comparison removed per request
