"""Floating chatbot widget component for the FedSIG+ dashboard."""
...
def fetch_threats_for_chat():
    """Fetch signature contribution data for chatbot responses with pagination."""
    try:
...
def process_user_query(query: str, df: pd.DataFrame, summary: Dict) -> str:
    ...
    latest_patterns = ["latest", "recent", "new", "show threats", "view threats", "recent threats", "new threats", "latest signatures", "new signatures"]
    risk_patterns = ["highest risk", "which client", "most threats", "riskiest", "highest-risk", "worst client", "most dangerous", "low trust", "noisy client"]
    health_patterns = ["health", "status", "system", "how is", "system status", "overall health", "trust status"]
    summary_patterns = ["summary", "summarize", "alerts", "overview", "latest alerts", "dashboard summary", "signature summary"]
    ...
    if any(word in text for word in latest_patterns) or fuzzy_match(text, latest_patterns) > 0.6:
        if not df.empty:
            ...
            if 'client_id' in latest.columns and 'file_path' in latest.columns:
                ...
                lines = [f"• **{r['client_id']}**: {r.get('file_path', 'N/A')} {'⚠️ Signature Alert' if r.get('is_threat') else '✓ Benign'}" for r in records]
                return "🛡️ **Latest 5 Signature Events:**\n\n" + "\n".join(lines)
            else:
                return f"🛡️ **Latest Events:**\n\nFound {len(latest)} recent signature events."
        return "No signature contributions recorded yet."
    ...
    if any(phrase in text for phrase in risk_patterns) or fuzzy_match(text, risk_patterns) > 0.6:
        if not df.empty and 'client_id' in df.columns:
            ...
            if not risk.empty:
                ...
                return f"🔴 **Highest Trust-Risk Client:** {top}\n\n• Signature Alerts: {count}\n• Total Events: {total_events}\n• Risk Rate: {risk_pct:.1f}%"
    ...
    if any(word in text for word in summary_patterns) or fuzzy_match(text, summary_patterns) > 0.6:
        ...
        isolations = int(summary.get('isolations', 0))
        recent_threats = int(summary.get('recent_threats_24h', 0))
        return f"📊 **Dashboard Summary:**\n\n• Trusted Clients: {clients}\n• Signature Alerts: {threats}\n• Recent Alerts (24h): {recent_threats}\n• Quarantines: {isolations}"
    
    # Help with enhanced matching
    if any(word in text for word in help_patterns) or fuzzy_match(text, help_patterns) > 0.6:
        return """**Available Commands:**

• "Show latest signatures" - View recent contributions
• "Which client has highest risk?" - Find the noisiest contributor
• "System health" - Check overall trust status
• "Summarize alerts" - Get KPI overview
• "Help" - Show this guide

**Tips:**
You can still ask about "threats"—I'll translate it to signature terminology automatically!"""
    
    # Default response with suggestions
    return "I can help with: latest signatures, system health, highest risk client, or alerts summary. Try asking:\n\n• 'Show latest signatures'\n• 'System health'\n• 'Which client has highest risk?'\n• 'Summarize today's alerts'\n\nType 'help' for more options."
...
def render():
    ...
    if 'chatbot_messages' not in st.session_state:
        st.session_state.chatbot_messages = [
            {"role": "assistant", "content": "Hi! I'm your FedSIG+ Assistant. How can I help you today?"}
        ]
    ...
    if not st.session_state.chatbot_open:
        # Toggle button
        if st.button("🤖", key="chatbot_toggle_btn", help="Open FedSIG+ Assistant"):
        ...
        <script>
            (function() {
                const btn = document.querySelector('[data-testid="baseButton-secondary"][aria-label*="Open FedSIG+"]');
                if (btn) {
