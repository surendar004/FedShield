...
def render(df: pd.DataFrame, key: Optional[str] = None):
    ...
    # Derive signature categories client-side without changing backend logic.
    ...
    fig.update_layout(
        title=dict(
            text='Signature Activity Segmentation',
    ...
    if time_col:
        try:
            ...
            if not df2.empty:
                # Group by time using an adaptive granularity so sparse data still
                # produces multiple points (use seconds/minutes/hours as needed).
                # Choose a bin size based on the overall time span of the data.
                time_min = df2[time_col].min()
                time_max = df2[time_col].max()
                span_seconds = (time_max - time_min).total_seconds() if pd.notnull(time_min) and pd.notnull(time_max) else 0
                if span_seconds <= 60:
                    freq = 's'   # seconds
                ...
                df2['hour'] = df2[time_col].dt.floor(freq)
                ...
                # If grouping produced too few points (e.g., 0 or 1), fall back to
                # grouping by the raw timestamp values to preserve more detail so
                # that lines (and spline smoothing) can be drawn.
                if len(hourly_threats) < 2:
                    hourly_threats = df2[df2['is_threat'] == True].groupby(time_col).size().reset_index(name='count')
                if len(hourly_all) < 2:
                    hourly_all = df2.groupby(time_col).size().reset_index(name='total')
                ...
                if not hourly_threats.empty:
                    fig_time.add_trace(go.Scatter(
                        ...
                        name='Signatures',
                        line_shape='spline',
                        line=dict(color='#FF1E56', width=3, smoothing=0.6),
                ...
                if not hourly_all.empty:
                    fig_time.add_trace(go.Scatter(
                        ...
                        name='Total Events',
                        line_shape='spline',
                        line=dict(color='#00FFFF', width=2, dash='dash', smoothing=0.6),
                        marker=dict(size=6, color='#00FFFF')
                    ))
                
                fig_time.update_layout(
                    title=dict(
                        text='Signature Activity Over Time',
                        font=dict(color='#E0F7FF', size=16, family='Inter'),
                        x=0.02,
                    ),
                    xaxis=dict(
