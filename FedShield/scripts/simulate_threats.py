"""
FedSIG+ signature simulator that POSTs randomized events to the local API every minute.
Does not modify server logic; uses the legacy /api/report_threat endpoint.
"""
...
def random_event():
    ...
    # Mark signature alert with some probability (same logic as original prototype)
...
def main():
    print("Starting FedSIG+ signature simulator. Posting an event every 60 seconds. Press Ctrl+C to stop.")
    while True:
        payload = random_event()
        try:
            r = requests.post(API, json=payload, timeout=5)
            print(f"[{datetime.utcnow().isoformat()}Z] POST {r.status_code} -> {payload['client_id']} {payload['file_path']} signature_alert={payload['is_threat']}")
