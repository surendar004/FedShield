...
# Import common third-party modules lazily; sklearn will be imported only when
# training or loading the model to reduce per-process DLL loads.
try:
...
def train_and_save(csv_path=None):
    ...
    # Import sklearn IsolationForest here to avoid importing heavy C extensions
    # at module import time (reduces memory/paging pressure when many clients run).
    try:
        _sklearn_ensemble = importlib.import_module('sklearn.ensemble')
        IsolationForest = _sklearn_ensemble.IsolationForest
    except Exception as e:
        raise RuntimeError('scikit-learn is required to train the model: %s' % e)
...
def load_local_model():
    ...
    try:
        ...
        # Propagate the exception so callers (clients) can fallback to heuristics
