...
class ThreatReportSchema(Schema):
    """Schema for FedSIG+ signature contributions (legacy field names retained)."""
    ...
    threat_type = fields.Str(
        load_default=None,
        allow_none=True,
        validate=validate.OneOf(['malware', 'unauthorized_access', 'data_leak', 'phishing', 'system_anomaly']),
        error_messages={'invalid': 'threat_type must be one of: malware, unauthorized_access, data_leak, phishing, system_anomaly'}
    )
    threat_name = fields.Str(
        load_default=None,
        allow_none=True,
        error_messages={'invalid': 'threat_name must be a string'}
    )


class ThreatQuerySchema(Schema):
    """Schema for querying the global signature pool."""
    page = fields.Int(
        load_default=1,
